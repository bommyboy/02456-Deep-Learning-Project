...
echo "Enhancing audio using STOCK DeepFilterNet2 (df2)"
...
output_dir="${data_dir}/enhanced_df2"
...
do
    ...
    do
        ...
        # python3 ${path_to_enhance} -m DeepFilterNet2 ${noisy_file_dir}/noisy${num}_SNRdb_${snr}_clnsp${num}.wav -o ${output_dir}
        # With --compensate-delay
        python3 ${path_to_enhance} -m DeepFilterNet2 ${noisy_file_dir}/noisy${num}_SNRdb_${snr}_clnsp${num}.wav -o ${output_dir} ${options}
    done
done